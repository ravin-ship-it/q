...
# --- Directories ---
INSTALL_DIR="$HOME/.local/share/q"
...
for file in "${files[@]}"; do
    ...
    if [ -f "$SCRIPT_SRC_DIR/$file" ]; then
        echo -e "${C_CYAN}📂 Copying $file from local source...${C_RESET}"
        cp "$SCRIPT_SRC_DIR/$file" "$dest"
    else
        echo -e "${C_CYAN}📥 Downloading $file from GitHub...${C_RESET}"
